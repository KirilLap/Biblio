...
if not exist %ISCC% (
    ...
    echo  Ожидаемый путь: C:\Program Files (x86)\Inno Setup 6\ISCC.exe
...
if "!CUR_VER!"=="" (
    echo  ОШИБКА: файл VERSION пустой!
    pause & exit /b 1
)
...
if "!BUILD_CLIENT!"=="0" if "!BUILD_ADMINWEB!"=="0" (
    echo  ОШИБКА: введи 1, 2 или 3
    pause & exit /b 1
)
...
echo.
echo  Версия !NEW_VER! сохранена в VERSION
...
echo.
echo ============================================
echo  [1/2 BibClient] dotnet publish...
echo ============================================
dotnet publish "%SCRIPT_DIR%BibClient\BibClient.csproj" -p:PublishProfile=win-x64
if errorlevel 1 (
    echo.
    echo  ОШИБКА: dotnet publish BibClient завершился с ошибкой!
    pause & exit /b 1
)

echo.
echo ============================================
echo  [2/2 BibClient] Inno Setup...
echo ============================================
%ISCC% /DAppVersion=!NEW_VER! "%SCRIPT_DIR%installer\bibclient-setup.iss"
if errorlevel 1 (
    echo.
    echo  ОШИБКА: Inno Setup bibclient-setup завершился с ошибкой!
    pause & exit /b 1
)
...
echo.
echo ============================================
echo  [1/2 BibAdminWeb] dotnet publish...
echo ============================================
dotnet publish "%SCRIPT_DIR%BibAdminWeb\BibAdminWeb.csproj" -p:PublishProfile=win-x64
if errorlevel 1 (
    echo.
    echo  ОШИБКА: dotnet publish BibAdminWeb завершился с ошибкой!
    pause & exit /b 1
)

echo.
echo ============================================
echo  [2/2 BibAdminWeb] Inno Setup...
echo ============================================
%ISCC% /DAppVersion=!NEW_VER! "%SCRIPT_DIR%installer\bibadminweb-setup.iss"
if errorlevel 1 (
    echo.
    echo  ОШИБКА: Inno Setup bibadminweb-setup завершился с ошибкой!
    pause & exit /b 1
)
